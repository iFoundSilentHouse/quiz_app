...
export default function ResultPage() {
  ...
  if (!result) return <div className="p-8 text-center text-gray-500">Загрузка результатов...</div>;

  return (
    <main className="max-w-3xl mx-auto p-6 space-y-8">
      <div className="text-center space-y-2">
        <h1 className="text-4xl font-black text-gray-900">Ваш результат</h1>
        <div className="text-6xl font-bold text-blue-600">
          {result.score} <span className="text-2xl text-gray-400">/ {result.results.length}</span>
        </div>
        <p className="text-gray-500 italic">Сделано учеником: {result.studentName}</p>
      </div>
      ...
      <div className="space-y-6">
        {result.results.map((ans, idx) => (
          <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex gap-6 items-center">
            <span className="text-xl font-bold text-gray-900">Вопрос {idx + 1}:</span>
            <img src={ans.imageUrl} className="w-32 h-32 object-cover rounded-lg flex-shrink-0" alt="" />
            <div className="flex-grow space-y-3">
              <div>
                <span className="text-xs font-bold text-gray-400 uppercase">Ваш ответ:</span>
                <DiffHighlight diff={ans.diff} />
              </div>
              <div>
                <span className="text-xs font-bold text-gray-400 uppercase">Правильно:</span>
